...
Next, we save this preprocessed data to the disc.
...
Next, we join the concordancing data and the bio-demographic data and add the freqeuncy of adjectives by age group to the data.
...
Next, we code whether or not an amplifier occured in a primed context, i.e. a context in which the same amplifier had occurred in up to three slots preceding the current slot and also define patterns that, if they occurred, in the context of the adjective in question, represent a reason for removing the adjective from the analysis (e.g. if the adjective was negated).
...
Next, we remove the adjectives that occurred in contexts that identify their contexts as being not isntances of the variable context. To do this, we cerate a vector of indices of problematic adjectives that we then remove from the analysis.
...
Next, we save the data to the disc and determine if an adjective was at all amplified or if it was amplified by at least to different types of amplifiers. Only adjectives that were amplified by at elast two different amplifiers are consiedred here to exclude fixed expressions such as *right honorable*.
...
Next, we create a vector of adjective that are either tagged errorneously or represent misspellings. In a next step, these adjectives are removed from the analysis.
