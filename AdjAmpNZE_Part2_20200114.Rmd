...
Next, we clean the data. Here, we recode age as the original age groups would have too few speakers to warrant meaningful claims about the language use.
...
ampwsc$Age <- ifelse(ampwsc$Age == "16-19", "16-19",
                     ifelse(ampwsc$Age == "20-24", "20-29",
                            ...
                            ifelse(ampwsc$Age == "45-49", "40-49", "50+"))))))) 
...
Next, we create a copy of the data for data visualization, and we also inspect the amplifier variants and their frequency.
...
# start plot: all
p2 <- ggplot(p2d, aes(x = jitter(Age, factor = .5), y = very)) +
...
p2bd$Age <- ifelse(p2bd$Age == "16-19", 5,
                 ifelse(p2bd$Age == "20-29", 4,
                        ifelse(p2bd$Age == "30-39", 3, 
                               ifelse(p2bd$Age == "40-49", 2, 
                                      ifelse(p2bd$Age == "50+", 1, p2bd$Age)))))
p2bd$Age <- as.numeric(p2bd$Age)
# plot
p2b <- ggplot(p2bd, aes(x = jitter(Age, factor = .5), y = zero)) +
  facet_wrap(vars(Function)) +
  geom_smooth(aes(y = other, color = "other", linetype = "other"), se = F, size = .5) +
  geom_smooth(aes(y = pretty, color = "pretty", linetype = "pretty"), se = F, size = .5) +
  geom_smooth(aes(y = really, color = "really", linetype = "really"), se = F, size = .5) +
  ...
  guides(size = FALSE)+
  guides(alpha = FALSE) +
  scale_x_discrete(name ="Age", limits = 1:5, labels= agelevels) +
  ggsave(file = paste(imageDirectory,"Variant_Function_Frequency1.png", sep="/"), 
         width = 15, height = 10, units = c("cm"),  dpi = 320)
p2b
```

Now, we plot the percentages of the most prominent amplifiers by age and function.
...
p3bd <- dplyr::left_join(wordcounts, ampwsc, by = c("File", "Speaker")) %>%
  ...
  dplyr::select(-File, -Speaker, -Wordcount) %>%
  dplyr::mutate(Age = factor(Age), Function = factor(Function))
p3bd$Age <- ifelse(p3bd$Age == "16-19", 5,
                   ifelse(p3bd$Age == "20-29", 4,
                          ifelse(p3bd$Age == "30-39", 3, 
                                 ifelse(p3bd$Age == "40-49", 2, 
                                        ifelse(p3bd$Age == "50+", 1, p3bd$Age)))))
...
Now, we turn to the lexical diversity of amplifiers to check if changes in frequency proceed or follow changes in the lexical diversity of variants. We begin by tabulating the frequency of amplifiers.
...
p4d$Age <- ifelse(p4d$Age == "16-19", 5,
                  ifelse(p4d$Age == "20-29", 4,
                         ifelse(p4d$Age == "30-39", 3, 
                                ifelse(p4d$Age == "40-49", 2, 
                                       ifelse(p4d$Age == "50+", 1, p4d$Age)))))
...
p5d <- ccd3 %>%
  ...
  tidyr::spread(Variant, Effect)
# create a numeric variable from Age
p5d$Age <- ifelse(p5d$Age == "16-19", 5,
                  ifelse(p5d$Age == "20-29", 4,
                         ifelse(p5d$Age == "30-39", 3, 
                                ifelse(p5d$Age == "40-49", 2, 
                                       ifelse(p5d$Age == "50+", 1, p5d$Age)))))
...
AdjectiveChangeData$Age <- ifelse(AdjectiveChangeData$Age == "16-19", 5,
                                  ifelse(AdjectiveChangeData$Age == "20-29", 4,
                                         ifelse(AdjectiveChangeData$Age == "30-39", 3, 
                                                ifelse(AdjectiveChangeData$Age == "40-49", 2, 
                                                       ifelse(AdjectiveChangeData$Age == "50+", 1, AdjectiveChangeData$Age)))))
...
Next, we turn to an analysis of changes in adjective frequency and visualize the frequencies across age groups. 
...
AmplifiedAdjectivesData$Age <- ifelse(AmplifiedAdjectivesData$Age == "16-19", 5,
                                      ifelse(AmplifiedAdjectivesData$Age == "20-29", 4,
                                             ifelse(AmplifiedAdjectivesData$Age == "30-39", 3, 
                                                    ifelse(AmplifiedAdjectivesData$Age == "40-49", 2, 
                                                           ifelse(AmplifiedAdjectivesData$Age == "50+", 1, AmplifiedAdjectivesData$Age)))))
