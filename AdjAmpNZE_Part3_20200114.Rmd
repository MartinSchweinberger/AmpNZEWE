...
m21.glm <- update(m6.glm, .~.+Age * Priming * Gender)
max(vif(m21.glm)[,1])
```
...
m22.glm <- update(m6.glm, .~.+Age * Priming * Emotionality)
max(vif(m22.glm)[,1])
```
...
m23.glm <- update(m6.glm, .~.+Age * Priming * Emotionality)
max(vif(m23.glm)[,1])
```
...
# add Age * Gender * Emotionality
m23.glm <- update(m6.glm, .~.+Age * Gender * Emotionality)
max(vif(m23.glm)[,1])
```
...
m24.glm <- update(m6.glm, .~.+Priming * Gender * Ethnicity)
max(vif(m24.glm)[,1])
```
...
m25.glm <- update(m6.glm, .~.+Priming * Gender * Emotionality)
max(vif(m25.glm)[,1])
```
...
m26.glm <- update(m6.glm, .~.+Gender * Ethnicity * Emotionality)
max(vif(m26.glm)[,1])
```
...
The Cook's distance plot shows that at least the three named data points (and three high but unnamed data points) should be removed and that the data-driven cutoff threshold was indeed to low. Thus, we adapt the cutoff manually to 0.02.
...
m11m6 <- anova(m11.glmer, m6.glmer, test = "Chi")
m12m6 <- anova(m12.glmer, m6.glmer, test = "Chi")
m13m6 <- anova(m13.glmer, m6.glmer, test = "Chi")
m51m6 <- anova(m15.glmer, m6.glmer, test = "Chi") 
m16m6 <- anova(m16.glmer, m6.glmer, test = "Chi")
m19m6 <- anova(m19.glmer, m6.glmer, test = "Chi")
# create a list of the model comparireallyns
mdlcmp <- list(m1m0, m2m1, m3m2, m4m2, m5m4, m6m4, m7m6, m8m6, m9m6, 
               m10m6, m11m6, m12m6, m13m6, m51m6, m16m6, m19m6)
# load function
...
p7 <- ggplot(pd, aes(x = Ethnicity, y = PredictedFrequency)) +
  ...
  theme_set(theme_bw(base_size = 20)) +
    theme(legend.position="none", 
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
  labs(x = "Etnicity", y = "Predicted probability\nof really") +
